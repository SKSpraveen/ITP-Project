...
function CCTVAdvertisements() {
    const [cctvAdvertisements, setCCTVAdvertisements] = useState([]);
        ...
        axios.get("http://localhost:8070/Advertisement")
            .then((response) => {
                if (response.data.success) {
                    const filteredAds = response.data.advertisements.filter(ad => ad.type === "CCTV");
                    setCCTVAdvertisements(filteredAds);
                } else {
    ...
    return (
        <div>
            ...
            <div style={{backgroundColor:"rgba(16, 16, 16, 0.926)",height:"56px"}}>
                <h1 style={{fontSize:"35px",padding:"4px"}}>CCTV Packages</h1>
            </div>
            <br />
            <div className="gallery">
                {cctvAdvertisements.map((ad) => (
...
export default CCTVAdvertisements;
